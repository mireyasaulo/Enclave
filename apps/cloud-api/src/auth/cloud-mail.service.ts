...
export class CloudMailService {
  ...
  async sendVerificationCode(
    ...
    const fromAddress = this.config.get<string>("CLOUD_MAIL_FROM_ADDRESS");
    const fromName =
  ...
  private resolveTransporter(): Transporter | null {
    ...
    const host = this.config.get<string>("CLOUD_SMTP_HOST");
    if (!host) {
      this.logger.warn(
    ...
    const port = Number(this.config.get<string>("CLOUD_SMTP_PORT") ?? "465");
    const secureRaw = this.config.get<string>("CLOUD_SMTP_SECURE") ?? "true";
