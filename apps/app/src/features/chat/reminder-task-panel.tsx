...
function getReminderTaskBadgeLabel(t: Translator, task: ReminderTaskRecord) {
  if (task.kind === "habit") {
    return t(msg`长期`);
  }
...
function buildReminderTaskMeta(t: Translator, task: ReminderTaskRecord) {
  if (task.snoozedUntil) {
    return t(msg`已延后到 ${formatMessageTimestamp(task.snoozedUntil)}`);
  }

  const nextTimestamp = parseTimestamp(task.nextTriggerAt ?? task.dueAt);
  if (nextTimestamp != null) {
    return t(msg`下次提醒 ${formatMessageTimestamp(String(nextTimestamp))}`);
  }
