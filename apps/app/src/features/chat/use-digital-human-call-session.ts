  ...
  const turnMutation = useMutation({
    mutationFn: async () => {
      if (!sessionRef.current) {
        throw new Error(translateRuntimeMessage(msg`数字人通话尚未建立，请稍后再试。`));
      }

      if (!speech.recordedAudio) {
        throw new Error(translateRuntimeMessage(msg`请先录一段语音再试。`));
      }
    ...
    onError: (error) => {
      ...
      setSessionError(
        error instanceof Error
          ? error.message
          : translateRuntimeMessage(msg`数字人通话失败，请稍后再试。`),
      );
    },
  });
    ...
    void (async () => {
      try {
        ...
        setSessionError(
          error instanceof Error
            ? error.message
            : translateRuntimeMessage(msg`连接数字人失败，请稍后再试。`),
        );
      }
    })();
...
function resolveAutoplayBlockedCopy() {
  return isNativeMobileRuntime()
    ? translateRuntimeMessage(msg`系统拦截了自动播报，点“重播上一句”即可播放。`)
    : translateRuntimeMessage(msg`浏览器拦截了自动播报，点“重播上一句”即可播放。`);
}

function resolveDigitalHumanPlaybackFailedCopy() {
  return isNativeMobileRuntime()
    ? translateRuntimeMessage(msg`数字人语音已生成，但当前设备没有成功播放。可以点“重播上一句”再试。`)
    : translateRuntimeMessage(msg`数字人语音已生成，但浏览器没有成功播放。可以点“重播上一句”再试。`);
}
