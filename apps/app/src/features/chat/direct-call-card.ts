...
export function resolveDirectCallStatusLabel(
  t: Translator,
  ...
  if (invite.connectionStatus === "connected") {
    return invite.kind === "video" ? t(msg`画面已接通`) : t(msg`已接通`);
  }

  return invite.kind === "video" ? t(msg`等待接入画面`) : t(msg`等待接听`);
}
...
export function resolveDirectCallFooterCopy(
  ...
  return canReopenCall
    ? {
        ...
        actionLabel: invite.kind === "voice" ? t(msg`回到语音`) : t(msg`回到视频`),
        tone: "info" as const,
    ...
    : {
        ...
        actionLabel: invite.kind === "voice" ? t(msg`语音中`) : t(msg`视频中`),
        tone: "info" as const,
