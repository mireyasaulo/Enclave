...
function buildMessages(state: FarmPlayerStateView, nowMs: number): string[] {
  ...
  if (ripeCount > 0) {
    messages.push(`喵——有 ${ripeCount} 块田熟了，趁热收吧。`);
  }
  if (rottenCount > 0) {
    messages.push(`糟糕，${rottenCount} 块田已经坏掉了，铲了重种？`);
  }
  if (weedCount > 0 || bugCount > 0) {
    messages.push(
      `田里${weedCount > 0 ? ` ${weedCount} 处杂草` : ""}${
        weedCount > 0 && bugCount > 0 ? "、" : ""
      }${bugCount > 0 ? ` ${bugCount} 只虫子` : ""}，记得清一下。`,
    );
  }
  if (stolenRecently.length > 0) {
    const first = stolenRecently[0]!;
    messages.push(`提醒一下：${first.thiefName} 今天来顺过你的菜。`);
  }

  // 时段补一句
  if (hour < 6) {
    messages.push("夜深了，作物也在睡觉，浇个水就回去歇会儿吧。");
  } else if (hour < 11) {
    messages.push("早上阳光好，正适合浇水。");
  } else if (hour < 14) {
    messages.push("中午阳光毒，浇水容易蒸发，等下午吧。");
  } else if (hour < 19) {
    messages.push("下午是好时候，串串门、看看邻居？");
  } else {
    messages.push("入夜啦，记得防一下夜里来串门的人。");
  }

  // 兜底
  if (messages.length === 0) {
    messages.push("今天没啥事儿，世界角色们各自忙各自的。");
  }
