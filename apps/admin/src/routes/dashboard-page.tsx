...
export function DashboardPage() {
  ...
  const [worldLanguageDraft, setWorldLanguageDraft] =
    useState<WorldLanguageCode>("zh-CN");
  ...
  const worldLanguageMutation = useMutation({
    ...
    onSuccess: async (result) => {
      setWorldLanguageDraft(result.language);
      setSuccessNotice(t(msg`世界语言已切换为 ${result.nativeLabel}。`));
      await Promise.all([
        queryClient.invalidateQueries({
          queryKey: ["admin-world-language", baseUrl],
        }),
        ...
        queryClient.invalidateQueries({
          queryKey: ["admin-reply-logic-overview"],
        }),
        queryClient.invalidateQueries({
          queryKey: ["admin-reply-logic-rules"],
        }),
      ]);
    },
  });
  ...
  const nextActionMessage = !desktopRuntimeReady
    ? t(
        msg`运行时尚未完全恢复。先进入设置页确认远程 API、运行数据目录和桌面托管状态。`,
      )
    : !providerConfigured
  ...
  const totalCharacters =
    ...
    statusQuery.data?.worldSurface.charactersCount ??
    0;
  const onlineCharacterCount = previewCharacters.filter(
    (item) => item.isOnline,
  ).length;
  const ownerCount =
    statusQuery.data?.worldSurface.ownerCount ??
    adminStatsQuery.data?.ownerCount ??
    null;
  const availableModelCount = availableModelsQuery.data?.models.length ?? 0;
  const schedulerJobs = schedulerQuery.data?.jobs ?? [];
  const enabledSchedulerJobCount = schedulerJobs.filter(
    (job) => job.enabled,
  ).length;
  const recentSchedulerErrorCount = (
    schedulerQuery.data?.recentRuns ?? []
  ).filter((run) => run.status === "error").length;
  ...
  const overallTone =
    blockerCount > 0 || watchItemCount > 0 ? "warning" : "healthy";
  const overallLabel =
    blockerCount > 0
  ...
  if (desktopAvailable && !desktopRuntimeReady) {
    dutyIssues.push({
      ...
      description: t(
        msg`桌面运行数据目录或受管 Core API 还没有完全恢复，桌面侧运维能力暂不可靠。`,
      ),
      statusLabel: t(msg`高优先级`),
      tone: "warning",
      meta:
        desktopStatusQuery.data?.message ??
        t(msg`优先进入设置页检查桌面托管状态和运行路径。`),
    ...
    dutyIssues.push({
      ...
      description: t(
        msg`核心接口还未恢复，后续角色、内容和调度数据都不适合作为稳定依据。`,
      ),
      statusLabel: t(msg`高优先级`),
      tone: "warning",
  ...
  if (!providerConfigured) {
    dutyIssues.push({
      ...
      description: t(
        msg`回复、评测和自动内容生成都会受到影响，运营动作缺乏可验证输出。`,
      ),
      statusLabel: t(msg`高优先级`),
      tone: "warning",
      meta:
  ...
  if (ownerCount !== null && ownerCount !== 1) {
    dutyIssues.push({
      ...
      description: t(
        msg`当前实例不满足单世界主人约束，需要先确认数据状态，再继续做运营操作。`,
      ),
  ...
  if (hasSchedulerIssues) {
    dutyIssues.push({
      ...
      description: t(
        msg`最近 ${recentSchedulerErrorCount} 条调度记录返回 error，建议先复核 job 状态和最近执行摘要。`,
      ),
      statusLabel: t(msg`关注`),
      tone: "warning",
  ...
  if (hasEvalFailures) {
    dutyIssues.push({
      ...
      description: t(
        msg`最近共有 ${failedEvalCount} 条失败评测，建议先确认是 provider 波动还是提示链路问题。`,
      ),
      statusLabel: t(msg`关注`),
      tone: "warning",
      meta: t(
        msg`总运行 ${evalOverviewQuery.data?.runCount ?? 0} 次，trace ${evalOverviewQuery.data?.traceCount ?? 0} 条`,
      ),
  ...
  useEffect(() => {
    if (worldLanguageQuery.data?.language) {
      setWorldLanguageDraft(worldLanguageQuery.data.language);
    }
  }, [worldLanguageQuery.data?.language]);
  ...
  const worldLanguageDirty = worldLanguageDraft !== activeWorldLanguage;
  const worldLanguageLabel =
    worldLanguageQuery.data?.nativeLabel ??
    worldLanguageOptions.find((item) => item.code === activeWorldLanguage)
      ?.nativeLabel ??
    activeWorldLanguage;
        ...
        <div className="order-2 space-y-6 xl:order-1">
            ...
            <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
              <Card className="bg-[color:var(--surface-console)]">
                <AdminSectionHeader
                  ...
                  actions={
                    <StatusPill
                      tone={dutyIssues.length ? "warning" : "healthy"}
                    >
                      {dutyIssues.length
                        ? t(msg`${dutyIssues.length} 项`)
                        : t(msg`当前无阻塞`)}
                    </StatusPill>
                  }
                />
                ...
                <div className="mt-4 space-y-3">
                  {dutyIssues.length ? (
                    ...
                    <AdminCallout
                      ...
                      description={t(
                        msg`实例已经满足继续运营的基础条件，可以把注意力转向评测验证、角色巡检和内容抽查。`,
                      )}
                      actions={
                        <Link to="/evals">
              ...
              <Card className="bg-[color:var(--surface-console)] xl:col-span-2">
                <AdminSectionHeader
                  ...
                  actions={
                    <StatusPill
                      tone={worldLanguageDirty ? "warning" : "healthy"}
                    >
                      {worldLanguageDirty ? t(msg`未保存`) : worldLanguageLabel}
                    </StatusPill>
                  }
                />
                <p className="mt-2 text-sm leading-6 text-[color:var(--text-secondary)]">
                  {t(
                    msg`这里切换的是世界运行语言，不是后台界面语言。保存后，新的聊天回复、朋友圈、评论、广场动态、视频号、开场白、主动消息和语音链路都会按目标语言生成；历史内容不会回写。`,
                  )}
                </p>
                ...
                <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.75fr)]">
                  <AdminSelectField
                    ...
                    onChange={(value) =>
                      setWorldLanguageDraft(value as WorldLanguageCode)
                    }
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                    <MetricCard
                      ...
                      detail={
                        worldLanguageQuery.data?.changedAt
                          ? t(
                              msg`上次切换 ${worldLanguageQuery.data.changedAt}`,
                            )
                          : t(msg`默认世界语言`)
                      }
                    />
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <Button
                    ...
                    className="rounded-2xl"
                    disabled={
                      worldLanguageMutation.isPending ||
                      worldLanguageQuery.isLoading ||
                      !worldLanguageDirty
                    }
                    onClick={() =>
                      worldLanguageMutation.mutate(worldLanguageDraft)
                    }
                  >
                    {worldLanguageMutation.isPending
                      ? t(msg`保存中...`)
                  ...
                  <Button
                    ...
                    className="rounded-2xl"
                    disabled={
                      worldLanguageMutation.isPending || !worldLanguageDirty
                    }
                    onClick={() => setWorldLanguageDraft(activeWorldLanguage)}
                  >
                    {t(msg`恢复当前值`)}
                  </Button>
                </div>

                {worldLanguageQuery.isError &&
                worldLanguageQuery.error instanceof Error ? (
                  <ErrorBlock
                    className="mt-4"
                    message={worldLanguageQuery.error.message}
                  />
                ) : null}
                {worldLanguageMutation.isError &&
                worldLanguageMutation.error instanceof Error ? (
                  <ErrorBlock
                    className="mt-4"
                    message={worldLanguageMutation.error.message}
                  />
                ) : null}

                <InlineNotice className="mt-4" tone="muted">
                  {t(
                    msg`语言约束会注入最终生成 Prompt 的末尾，用来覆盖角色旧提示词里可能残留的中文回复要求。`,
                  )}
                </InlineNotice>
              </Card>
            </div>
          ...
          <div ref={runtimeSectionRef} className="scroll-mt-24 space-y-4">
            <DashboardSectionLead
              eyebrow={t(msg`运行体征`)}
              title={t(
                msg`把运行链路、模型接入和数字人可用性放在同一个工作区。`,
              )}
              description={t(
                msg`运营先确认实例连通和模型可用，再决定是否继续做内容抽查、角色观察或回复排障。`,
              )}
            />
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              <Card className="bg-[color:var(--surface-console)]">
                ...
                <p className="mt-2 text-sm leading-6 text-[color:var(--text-secondary)]">
                  {t(
                    msg`桌面壳模式下这里负责确认托管状态；远程模式下这里主要看 Core API 和运行目录是否稳定。`,
                  )}
                </p>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  ...
                  <MetricCard
                    ...
                    value={
                      runtimeContextQuery.data?.runtimeDataDir ??
                      (desktopAvailable ? t(msg`加载中`) : t(msg`远程实例`))
                    }
                    detail={
                      runtimeContextQuery.data?.databasePath ??
                      desktopStatusQuery.data?.databasePath ??
                      t(msg`等待诊断`)
                    }
                  />
                  <MetricCard
                ...
                <InlineNotice
                  className="mt-4"
                  tone={desktopRuntimeReady ? "success" : "warning"}
                >
                  {desktopAvailable
                    ? desktopRuntimeReady
                      ? t(
                          msg`桌面运行时已就绪。恢复、推理配置和手动管理入口已经统一收敛到设置页。`,
                        )
                      : (desktopStatusQuery.data?.message ??
                        t(
                          msg`桌面运行时尚未完成初始化，进入设置页可集中恢复。`,
                        ))
                    : systemHealthy
                      ? t(
                          msg`当前处于远程连接模式，Core API 健康，可继续做后台操作。`,
                        )
                      : t(
                          msg`当前为远程连接模式，但 Core API 还未健康，优先排查设置页与实例连通性。`,
                        )}
                </InlineNotice>
              ...
              <Card className="bg-[color:var(--surface-console)]">
                <AdminSectionHeader
                  ...
                  actions={
                    <StatusPill
                      tone={providerConfigured ? "healthy" : "warning"}
                    >
                      {providerConfigured ? t(msg`可验证`) : t(msg`待配置`)}
                    </StatusPill>
                  }
                />
                <p className="mt-2 text-sm leading-6 text-[color:var(--text-secondary)]">
                  {t(
                    msg`先确认模型、队列和数字人链路可用，再进入内容抽查或回复排障，避免把环境问题误判成业务问题。`,
                  )}
                </p>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  <MetricCard
                    ...
                    value={
                      ...
                      aiModelQuery.data?.model ??
                      t(msg`待配置`)
                    }
                    detail={
                      statusQuery.data?.inferenceGateway.activeProvider
                        ? t(
                            msg`网关：${statusQuery.data.inferenceGateway.activeProvider}`,
                          )
                  ...
                  <MetricCard
                    ...
                    detail={t(
                      msg`处理中 ${statusQuery.data?.inferenceGateway.inFlightRequests ?? 0} · 最大并发 ${statusQuery.data?.inferenceGateway.maxConcurrency ?? 0}`,
                    )}
                  />
                  <MetricCard
                    label={t(msg`最近成功时间`)}
                ...
                <InlineNotice
                  className="mt-4"
                  tone={
                    digitalHumanSummary.ready && providerConfigured
                      ? "success"
                      : "warning"
                  }
                >
                  {digitalHumanSummary.description}{" "}
          ...
          <div ref={signalsSectionRef} className="scroll-mt-24 space-y-4">
            <DashboardSectionLead
              eyebrow={t(msg`运营信号`)}
              title={t(
                msg`先看角色和内容有没有稳定产出，再决定是否深入到具体工作区。`,
              )}
              description={t(
                msg`这一块服务抽查而不是全面管理，所以只保留关键摘要和最新样本，减少视觉噪音。`,
              )}
            />
                  ...
                  {previewCharacters.length ? (
                    previewCharacters.map((character) => (
                      <ListItemCard
                        ...
                        actions={
                          <StatusPill
                            tone={character.isOnline ? "healthy" : "muted"}
                          >
                    ...
                    <AdminPanelEmpty
                      message={
                        charactersQuery.error instanceof Error
                          ...
                          : t(
                              msg`新核心接口进程启动后，角色增删改查兼容路由即可正常使用。`,
                            )
                      }
                    />
                  )}
          ...
          <div ref={schedulerSectionRef} className="scroll-mt-24 space-y-4">
            <DashboardSectionLead
              ...
              description={t(
                msg`先看最近有没有失败，再决定是手动触发 job，还是排查实时房间与客户端连接。`,
              )}
            />
            ...
            <AdminCallout
              ...
              title={
                hasSchedulerIssues
                  ? t(msg`最近调度存在异常，建议优先排查`)
                  : t(msg`最近调度稳定，可继续日常巡检`)
              }
              description={
                hasSchedulerIssues
                  ? recentSchedulerRuns.find((run) => run.status === "error")
                      ?.summary
                    ? translateSchedulerSummary(
                        recentSchedulerRuns.find(
                          (run) => run.status === "error",
                        )?.summary,
                        t,
                      )
                    : t(
                        msg`最近存在调度失败记录，请先检查 job 状态和最近执行摘要。`,
                      )
                  : t(
                      msg`当前没有新的调度失败记录，若内容产出异常，可先手动执行关键 job 做一次验证。`,
                    )
              }
            />
            ...
            <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
              <Card className="bg-[color:var(--surface-console)]">
                ...
                <p className="mt-2 text-sm leading-6 text-[color:var(--text-secondary)]">
                  {t(
                    msg`把 job 状态、手动执行和最近记录收敛在同一块，减少来回滚动和切页成本。`,
                  )}
                </p>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-3">
                  ...
                  <MetricCard
                    ...
                    detail={t(msg`总任务 ${schedulerJobs.length}`)}
                  />
                  <MetricCard
                    label={t(msg`最近失败`)}
                    value={recentSchedulerErrorCount}
                    detail={t(
                      msg`最近运行 ${schedulerQuery.data?.recentRuns.length ?? 0}`,
                    )}
                  />
                </div>
                ...
                <div className="mt-4 grid gap-3">
                  {schedulerJobs.map((job) => (
                    <ListItemCard
                        ...
                        <>
                          ...
                          <Button
                            ...
                            disabled={
                              !job.enabled ||
                              job.running ||
                              schedulerRunMutation.isPending
                            }
                            onClick={() => schedulerRunMutation.mutate(job.id)}
                          >
                            {runningSchedulerJobId === job.id
                              ? t(msg`执行中...`)
                              : t(msg`立即执行`)}
                          </Button>
                        </>
                      ...
                      body={
                        <>
                          <div>
                            {translateSchedulerJobDescription(
                              job.id,
                              job.description,
                              t,
                            )}
                  ...
                  {!schedulerJobs.length && !schedulerQuery.error ? (
                    <AdminPanelEmpty message={t(msg`等待调度器对齐数据...`)} />
                  ) : null}
                </div>
                  ...
                  {recentSchedulerRuns.length ? (
                    recentSchedulerRuns.map((event) => (
                      <ListItemCard
                        ...
                        body={
                          <div>
                            {translateSchedulerSummary(event.summary, t)}
                          </div>
                        }
                        footer={
              ...
              <Card className="bg-[color:var(--surface-console)]">
                <AdminSectionHeader
                  ...
                  actions={
                    <StatusPill
                      tone={
                        (realtimeQuery.data?.connectedClients ?? 0) > 0
                          ? "healthy"
                          : "muted"
                      }
                    >
                ...
                <p className="mt-2 text-sm leading-6 text-[color:var(--text-secondary)]">
                  {t(
                    msg`这里主要服务“为什么前端没同步到最新消息或状态”的排查，而不是日常高频操作。`,
                  )}
                </p>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <MetricCard
                    label={t(msg`已连接客户端`)}
                    value={realtimeQuery.data?.connectedClients ?? 0}
                  />
                  <MetricCard
                    label={t(msg`活跃房间`)}
                    value={realtimeQuery.data?.activeRooms ?? 0}
                  />
                </div>
                ...
                <AdminDetailPanel
                  ...
                  title={t(msg`命名空间与路径`)}
                >
                  {t(msg`命名空间`)}：{CHAT_NAMESPACE} · Socket {t(msg`路径`)}：
                  {realtimeQuery.data?.socketPath ?? "/socket.io"}
                </AdminDetailPanel>
                  ...
                  {previewRooms.length ? (
                    previewRooms.map((room) => (
                      <ListItemCard
                        ...
                        body={
                          <div>
                            {t(msg`订阅数`)}：{room.subscriberCount}
                          </div>
                        }
                      />
                    ))
            ...
            <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
              <Card className="bg-[color:var(--surface-console)]">
                <AdminSectionHeader
                  ...
                  actions={
                    <StatusPill
                      tone={previewMutation.data ? "healthy" : "muted"}
                    >
                ...
                >
                  {providerConfigQuery.isLoading
                    ...
                    : providerConfigQuery.isError &&
                        providerConfigQuery.error instanceof Error
                      ? providerConfigQuery.error.message
                      : providerConfigured
                        ? t(
                            msg`当前推理服务已配置，适合直接做联通性和基础输出验证。`,
                          )
                        : t(
                            msg`当前尚未配置推理服务。进入设置页可完成首轮配置并测试连通性。`,
                          )}
                </InlineNotice>
              ...
              <div className="space-y-6">
                <Card className="bg-[color:var(--surface-console)]">
                  <AdminSectionHeader
                    title={t(msg`运维操作`)}
                    actions={
                      <AdminMetaText>{t(msg`低频高风险`)}</AdminMetaText>
                    }
                  />
                  ...
                  <div className="mt-4 space-y-3">
                    {operationsBusy ? (
                      <AdminActionFeedback
                        ...
                        title={t(msg`运维任务执行中`)}
                        description={t(
                          msg`当前有运维任务执行中，其他维护操作暂时被锁定。`,
                        )}
                      />
                    ) : null}
                    {exportDiagnosticsMutation.isError &&
                    exportDiagnosticsMutation.error instanceof Error ? (
                      <ErrorBlock
                ...
                <Card className="bg-[color:var(--surface-console)]">
                  ...
                  <p className="mt-2 text-sm leading-6 text-[color:var(--text-secondary)]">
                    {t(
                      msg`保留最近日志入口，方便做故障留痕和快速定位，不再让日志列表挤占首页中段。`,
                    )}
                  </p>
                  ...
                  >
                    ...
                    {logsQuery.isLoading ? (
                      <LoadingBlock
                        className="border-0 bg-transparent px-0 py-0 text-left text-sm shadow-none"
                        label={t(msg`正在加载本地运行时日志...`)}
                      />
                    ) : null}
                    {!previewLogs.length && logsQuery.error instanceof Error ? (
                      <ErrorBlock message={logsQuery.error.message} />
                    ) : null}
                    {!logsQuery.isLoading &&
                    !previewLogs.length &&
                    !logsQuery.error ? (
                      <AdminPanelEmpty
                        message={t(msg`等待本地运行时日志...`)}
                      />
                    ) : null}
                  </AdminDetailPanel>
                </Card>
...
function translateSchedulerSummary(
  ...
  const heartbeatFindings = summary.match(
    /^heartbeat 命中了 (\d+) 类待处理事项。$/,
  );
...
function formatDesktopDiagnostics(
  values: {
    platform: string;
  ...
  const packageStatus = values.linuxMissingPackages.length
    ...
    : t(msg`Linux 依赖正常`);
  const sidecarStatus = formatCommandSource(
    values.coreApiCommandSource,
    values.bundledCoreApiExists,
    t,
  );
  const failureStatus =
