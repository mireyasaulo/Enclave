...
export function FollowupRuntimePage() {
  ...
  const attentionSummary = useMemo(() => {
    ...
    if (!draft?.enabled) {
      return {
        ...
        title: t(msg`主动跟进当前停用`),
        description: t(msg`调度不会真正处理新的闭环线索。建议先确认是否需要恢复启用，再决定是否执行手动调度。`),
      };
    }
    ...
    if (dirty) {
      return {
        tone: "info" as const,
        title: t(msg`存在未保存草稿`),
        description: t(msg`页面中的门槛、权重或文案已经调整，但服务端还在使用旧规则。先保存，再决定是否立即复跑。`),
      };
    }
    ...
    if (recentRun?.status === "failed") {
      return {
        tone: "warning" as const,
        title: t(msg`最近一轮运行失败`),
        description: t(msg`优先查看运行记录里的错误信息，确认是调度异常、规则冲突，还是下游写入失败。`),
      };
    }
    ...
    if (pendingFriendRequests > 0) {
      return {
        tone: "info" as const,
        title: t(msg`存在待确认的好友申请链路`),
        description: t(msg`当前有 ${pendingFriendRequests} 条推荐已经进入好友申请阶段，建议先核对这些链路是否继续推进。`),
      };
    }
    ...
    if (inProgressRecommendationCount > 0) {
      return {
        tone: "info" as const,
        title: t(msg`推荐闭环正在推进`),
        description: t(msg`最近有 ${inProgressRecommendationCount} 条推荐还在推进中，先看推荐闭环是否已经被打开、加友或转成聊天。`),
      };
    }
    ...
    if (attentionOpenLoopCount > 0) {
      return {
        tone: "warning" as const,
        title: t(msg`还有待判断的 open loop`),
        description: t(msg`当前仍有 ${attentionOpenLoopCount} 条线索处于观察或待推荐阶段，建议先看分数和来源是否与当前阈值匹配。`),
      };
    }

    return {
      tone: "success" as const,
      title: t(msg`当前闭环节奏稳定`),
      description: t(msg`没有明显堆积的待跟进事项。此时更适合微调阈值和文案，而不是大幅度改动策略。`),
    };
  ...
  const strategyRows = useMemo(() => {
    ...
    return [
      {
        label: t(msg`调度状态`),
        value: draft.enabled ? t(msg`启用中`) : t(msg`已停用`),
      },
      {
        label: t(msg`执行模式`),
      ...
      {
        label: t(msg`扫描节奏`),
        value: t(msg`${draft.scanIntervalMinutes} 分钟 / 回看 ${draft.lookbackHours} 小时`),
      },
      {
        label: t(msg`核心阈值`),
      ...
      {
        label: t(msg`最近执行`),
        value: latestRun ? formatDateTime(latestRun.startedAt) : t(msg`暂无`),
      },
    ];
  }, [draft, overviewQuery.data]);
  ...
  const actionFeedback = useMemo(() => {
    if (saveMutation.isPending) {
      return {
        tone: "busy" as const,
        title: t(msg`正在保存规则`),
        description: t(msg`规则更新会覆盖当前服务端配置，保存完成后会自动刷新总览数据。`),
      };
    }
    ...
    if (runMutation.isPending) {
      return {
        tone: "busy" as const,
        title: t(msg`正在执行主动跟进调度`),
        description: t(msg`本轮会重新扫描最近安静下来的线程，并刷新 open loop 与推荐结果。`),
      };
    }
  ...
  if (overviewQuery.isLoading && !overviewQuery.data) {
    return <LoadingBlock label={t(msg`正在读取主动跟进配置...`)} />;
  }
  ...
  if (!overviewQuery.data || !draft) {
    return <LoadingBlock label={t(msg`正在同步主动跟进工作台...`)} />;
  }
  ...
  return (
    <div className="space-y-6">
      <AdminPageHero
        ...
        actions={
          <>
            ...
            >
              {saveMutation.isPending ? t(msg`保存中...`) : t(msg`保存规则`)}
            </Button>
          </>
        }
      />
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr,0.85fr]">
        ...
        />
        <AdminInfoRows title={t(msg`当前策略快照`)} rows={strategyRows} />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.3fr,0.95fr]">
        <Card className="bg-[color:var(--surface-console)]">
          <AdminSectionHeader
            ...
            actions={
              <StatusPill tone={draft.enabled ? "healthy" : "warning"}>
                {draft.enabled ? t(msg`运行中`) : t(msg`停用中`)}
              </StatusPill>
            }
          />
          ...
          <AdminTabs
            className="mt-4"
            tabs={[
              { key: "open_loops", label: t(msg`Open Loop 队列`) },
              { key: "recommendations", label: t(msg`推荐闭环`) },
        ...
        <Card className="bg-[color:var(--surface-console)]">
          ...
          <AdminTabs
            className="mt-4"
            tabs={[
              { key: "policy", label: t(msg`调度门槛`) },
              { key: "weights", label: t(msg`候选打分`) },
              { key: "copy", label: t(msg`Prompt 与文案`) },
            ]}
            activeKey={configTab}
...
function OpenLoopWorkbench({
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      {!visibleLoops.length ? (
        <AdminEmptyState
          title={t(msg`当前没有对应的 open loop`)}
          description={t(msg`如果你刚执行过调度但这里还是空的，通常代表最近没有满足门槛的线索。`)}
        />
      ) : (
        <div className="grid gap-4 xl:grid-cols-[0.92fr,1.08fr]">
          <div className="space-y-3">
            {visibleLoops.map((loop) => (
              <AdminSelectableCard
                ...
                meta={t(msg`${labelFromLoopStatus(loop.status)} · 最近提及 ${formatDateTime(loop.lastMentionedAt)}`)}
                badge={
          ...
          {selectedLoop ? (
            ...
            <AdminEmptyState
              title={t(msg`没有可查看的线索`)}
              description={t(msg`切换筛选后如果为空，说明这个阶段当前没有数据。`)}
            />
          )}
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      <AdminRecordCard
        ...
        description={
          loop.reasonSummary ||
          t(msg`这条线索目前没有额外原因摘要，可以先结合分数和来源线程判断。`)
        }
      />
      ...
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <ScoreCard
          ...
          description={t(msg`当前阈值 ${formatScore(rules.minHandoffNeedScore)}`)}
          tone={
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      {!visibleRecommendations.length ? (
        <AdminEmptyState
          title={t(msg`当前没有对应的推荐记录`)}
          description={t(msg`如果刚执行过调度，这里为空通常代表暂时没有生成新的推荐。`)}
        />
      ) : (
        <div className="grid gap-4 xl:grid-cols-[0.9fr,1.1fr]">
          ...
          {selectedRecommendation ? (
            ...
            <AdminEmptyState
              title={t(msg`没有可查看的推荐`)}
              description={t(msg`切换筛选后如果为空，说明这一阶段当前没有数据。`)}
            />
          )}
        </div>
      )}
  ...
  return (
    <div className="space-y-4">
      ...
      <AdminRecordCard
        ...
        meta={t(msg`${labelFromThreadType(recommendation.sourceThreadType)} · ${recommendation.sourceThreadTitle || recommendation.sourceThreadId}`)}
...
function RunWorkbench({ runs }: { runs: FollowupRunRecord[] }) {
  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      {!visibleRuns.length ? (
        <AdminEmptyState
          title={t(msg`当前没有对应的运行记录`)}
          description={t(msg`如果这里为空，说明最近没有触发主动跟进调度。`)}
        />
      ) : (
        <div className="grid gap-4 xl:grid-cols-[0.9fr,1.1fr]">
          <div className="space-y-3">
            {visibleRuns.map((run) => (
              <AdminSelectableCard
                ...
                meta={t(msg`候选 ${run.candidateLoopCount} / 选中 ${run.selectedLoopCount} / 发出 ${run.emittedRecommendationCount}`)}
                badge={
          ...
          {selectedRun ? (
            ...
            <AdminEmptyState
              title={t(msg`没有可查看的运行`)}
              description={t(msg`切换筛选后如果为空，说明这一阶段当前没有记录。`)}
            />
          )}
        </div>
      )}
...
function RunDetail({ run }: { run: FollowupRunRecord }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <AdminRecordCard
        title={run.summary || t(msg`本轮未写入总结。`)}
        badges={
          <StatusPill tone={toneFromRunStatus(run.status)}>
            {labelFromRunStatus(run.status)}
          </StatusPill>
      ...
      {run.errorMessage ? (
        <AdminSoftBox className="text-xs leading-5 text-rose-700">
          {t(msg`错误信息：${run.errorMessage}`)}
        </AdminSoftBox>
      ) : null}
      {run.skipReason ? (
        <AdminSoftBox className="text-xs leading-5">
          {t(msg`跳过原因：${run.skipReason}`)}
        </AdminSoftBox>
      ) : null}
    </div>
  );
...
function PolicyConfigSection({
  ...
  return (
    <div className="mt-6 space-y-4">
      <ConfigGroup
        title={t(msg`运行开关`)}
        description={t(msg`先决定这套机制是否真正发消息，再决定遇到非好友时要不要自动补发好友申请。`)}
      >
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <SelectField
            ...
            options={[
              { value: "emit_messages", label: t(msg`直接发我自己消息 + 名片`) },
              { value: "dry_run", label: t(msg`只记录候选，不发消息`) },
            ]}
            onChange={(value) =>
              onChange({
                ...draft,
      ...
      <ConfigGroup
        title={t(msg`调度窗口`)}
        description={t(msg`控制多频繁扫描，以及多旧、多安静的线程会被视为值得再次回捞。`)}
      >
        <div className="grid gap-4 md:grid-cols-2">
          <NumberField
            label={t(msg`扫描间隔（分钟）`)}
      ...
      <ConfigGroup
        title={t(msg`吞吐与阈值`)}
        description={t(msg`限制每轮读取与发送的强度，并控制什么样的线索才值得进入 handoff 阶段。`)}
      >
        <div className="grid gap-4 md:grid-cols-2">
          <NumberField
            label={t(msg`每日推荐上限`)}
...
function labelFromThreadType(type: FollowupOpenLoopRecord["sourceThreadType"]) {
  const t = translateRuntimeMessage;
  return type === "group" ? t(msg`群聊`) : t(msg`私聊`);
}
...
function describeRecommendation(recommendation: FollowupRecommendationRecord) {
  ...
  switch (recommendation.status) {
    case "friend_request_pending":
      return {
        tone: "warning" as const,
        title: t(msg`推荐已经进入等待通过阶段`),
        description: t(msg`当前的关键不是继续多发，而是确认好友申请是否会被处理，避免重复打扰。`),
      };
    case "opened":
      return {
        tone: "info" as const,
        title: t(msg`推荐已被打开，但还未形成关系推进`),
        description: t(msg`说明内容已经被看到，下一步重点看是否需要更强的招呼语或更明确的角色匹配。`),
      };
    case "friend_added":
    case "chat_started":
    case "resolved":
      return {
        tone: "success" as const,
        title: t(msg`这条推荐已经完成主要转化`),
        description: t(msg`可以把它当成有效样本，回头观察推荐理由和关系匹配是否值得复制。`),
      };
    case "dismissed":
    case "expired":
      return {
        tone: "warning" as const,
        title: t(msg`这条推荐已经自然结束`),
        description: t(msg`适合回看原因摘要和目标角色定位，判断是时机不对，还是推荐对象不合适。`),
      };
    default:
      return {
        tone: "info" as const,
        title: t(msg`推荐正在推进中`),
        description: t(msg`优先核对来源线程、关系状态和目标角色定位，判断当前链路是否合理。`),
      };
  }
}
...
function describeRun(run: FollowupRunRecord) {
  ...
  if (run.status === "failed") {
    return {
      tone: "warning" as const,
      title: t(msg`这轮运行失败，需要先排查`),
      description: t(msg`先看错误信息和扫描窗口，再判断是调度异常、数据写入失败，还是下游链路不通。`),
    };
  }
  ...
  if (run.status === "skipped") {
    return {
      tone: "info" as const,
      title: t(msg`这轮运行被跳过`),
      description: t(msg`跳过不一定是坏事。优先看 skip reason，确认是主动停用、无信号，还是被容量门槛拦住。`),
    };
  }
  ...
  return {
    tone: "success" as const,
    title: t(msg`这轮运行完成`),
    description: t(msg`重点看候选数、选中数和发出数是否符合预期，用来判断当前门槛是不是过松或过紧。`),
  };
}
